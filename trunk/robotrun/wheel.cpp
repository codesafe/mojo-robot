...
bool	Wheel::reset()
{
	...
	if(torquelimit > -1)
	{
		...
		if( ret == false )
		{
			Logger::getInstance()->log("%d : torque limit recv error!! \n", id);
			return false;
		}

		if( torquelimit != param )
		{
			Logger::getInstance()->log("%d : torquelimit is not match : torquelimit : %d, recv torquelimit : %d\n", id, torquelimit, param);

			ret = Device::getInstance()->send(id, TORQUELIMIT, 2, torquelimit);
			if( ret == true )
			{
				Logger::getInstance()->log("%d : adjust torquelimit to %d \n", id, torquelimit);
			}
			else if( ret == false )
			{
				Logger::getInstance()->log("%d : set torquelimit send error!! \n", id);
				return false;
			}
		}
	}
